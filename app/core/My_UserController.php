...
class My_UserController extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!session('uid')){
            $this->session->set_flashdata('message',['message'=>'Please login again...', 'status'=>'info']);
            redirect(base_url('signin'));
        }   
        if (!in_array(segment(2), array('cron')) && !in_array(segment(3), array('cron', 'complete'))) {
            $allowed_controllers = ['auth', 'api','crons'];
    ...
    public function index()
    {
        ...
        if (in_array($this->controller_name, ['tickets','subscriptions'])) {
        ...
        $data = array(
            ...
            "pagination"          => create_pagination([
                'base_url'         => client_url($this->controller_name),
