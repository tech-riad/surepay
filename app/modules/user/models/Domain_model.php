...
class Domain_model extends MY_Model 
{
    ...
    public function get_item($params = null, $option = null)
    {
        ...
        if($option['task'] == 'get-item'){
            $result = $this->get("*", $this->tb_main, ['id' => $params['id'],'user_id'=>session('uid')], '', '', true);
        }
        if ($option['task'] == 'brand-list-items-on-invoice') {
            
            $result = $this->fetch("domain,id, brand_name", $this->tb_main, ['user_id'=>session('uid')], '', '', true);
        }
        return $result;
    }
